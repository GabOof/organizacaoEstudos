...
class MateriaDAOMongo extends MateriaDAOInterface {
  ...
  async encontrarMateriasPorEstudante(estudanteId) {
    try {
      ...
      console.error(
        `Erro ao buscar matérias do estudante ${estudanteId}: ${error.message}`
      );
  ...
  async atualizarMateria(materiaId, novosDados) {
    try {
      ...
      if (!materiaAtualizada) {
        throw new Error(
          `Matéria com ID ${materiaId} não encontrada para atualização`
        );
      }
      ...
      console.error(
        `Erro ao atualizar matéria com ID ${materiaId}: ${error.message}`
      );
  ...
  async excluirMateria(materiaId) {
    try {
      // Encontra e exclui a matéria pelo ID
      const materiaExcluida = await MateriaModel.findByIdAndDelete(materiaId);
      return materiaExcluida;
    } catch (error) {
      console.error("Erro ao excluir matéria no DAO:", error);
